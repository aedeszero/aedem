...
class Report(Base):
	...
	geolatitude = Column(String, nullable = True)
	geolongitude = Column(String, nullable = True)
	last_updated = Column(DateTime, onupdate = datetime.datetime.now)
	created_at = Column(DateTime, nullable = False, server_default = text('NOW()'))
	# id_ipaddr = Column(Integer, ForeignKey('ip_records.id'))
	# ipdarr = relationship("IPRecord", back_populates="reports")
	description = Column(String, nullable = True)
	attachments = relationship('Attachment', back_populates='report')

	def __init__(self, state_abbr, city_name, area, geolatitude, geolongitude, description):
